...
class Attention(nn.Module):
    ...
    def forward(
            ...
            past_key_value: Optional[Tuple[torch.Tensor]] = None,## [  kv,batch head, seq16 dim]???
        ...
        value_states = proj[2].view(bsz, q_len, self.num_heads, self.head_dim).transpose(1,
                                                                                         2)  # batch_size x source_len x hidden_size
        #[1b 4head 32seq 64dim]]
        kv_seq_len = key_states.shape[-2]
        if past_key_value is not None:
            #kv_seq_len += past_key_value[0].shape[-2]
            #kv_seq_len += 16
            prefixlen=past_key_value.shape[-2]
        cos, sin = self.rotary_emb(value_states, seq_len=kv_seq_len)# [1 1 48seq 64]
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids) # [1 4 32 64]
        # [bsz, nh, t, hd]

        if past_key_value is not None:
            # reuse k, v, self_attention
            key_states = torch.cat([past_key_value[0], key_states], dim=2) # [1 4 16 64],[1 4 32 64]->[1 4 48 64]
            value_states = torch.cat([past_key_value[1], value_states], dim=2)

        past_key_value = (key_states, value_states) if use_cache else None

        attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(self.head_dim) #[1 4head 32 48]

        if attn_weights.size() != (bsz, self.num_heads, q_len, kv_seq_len+prefixlen):
            raise ValueError(
        ...
        if attention_mask is not None:
            if attention_mask.size() != (bsz, 1, q_len, kv_seq_len+prefixlen):
                raise ValueError(
                    f"Attention mask should be of size {(bsz, 1, q_len, kv_seq_len+prefixlen)}, but is {attention_mask.size()}"
                )
        ...
        attn_output = torch.matmul(attn_weights, value_states) # [1 4 32 64]
...
class Model(PreTrainedModel):
    ...
    def forward(
        ...
        #past_key_values=torch.randn(size=[[2, 3, 1,4, 16,64])  # [kv,layer,batch,head,seq,dim]
        ...
        if position_ids is None:
            ...
            # ) #从正文开始 不包括prefix [16,,,,47]前16是prefix
            position_ids = torch.arange(
                0, seq_length , dtype=torch.long, device=device)  #[0,1,....31]
...
class BaiChuanForCausalLM(PreTrainedModel):
    ...
    def forward(
        ...
        ############# prefix each layer learnable,deep prompt ########
        #past_key_values =   [2,layer,batch,head,seq,dim]
